...
DBL_vol_YrMo=aggregate(DBL_vol_total_m3~year+month,FUN='mean',gb_elev_vol)
UBL_vol_YrMo=aggregate(UBL_vol_m3~year+month,FUN='mean',gb_elev_vol)
gb_vol_YrMo=merge(UBL_vol_YrMo,DBL_vol_YrMo,by=c("year","month"))
```
...
wq_data_flat=within(wq_data_flat, {
	bl=NA
	bl[(site_no==405356112205601 | site_no== 410644112382601) & Sampling_depth_m >=5] = 'DBL'
...
mon_sd=aggregate(value~year+month+variable+param+bl, gb_pool_wq_data, FUN='sd')
mon_means=reshape2::dcast(mon_means, year+month+variable+param~bl, value.var='value')
mon_sd=reshape2::dcast(mon_sd, year+month+variable+param~bl, value.var='value')
mon_means=dplyr::rename(mon_means, DBL_mean=DBL, UBL_mean=UBL)
mon_sd=dplyr::rename(mon_sd, DBL_sd=DBL, UBL_sd=UBL)
mon_means=merge(mon_means, mon_sd)
```
...
gb_pool_data=within(gb_pool_data, {
	...
	DBL_mass_kg=DBL_vol_total_m3*DBL_mean*0.001
...
jvl_dn_comp=plot_ly(data=comp) %>%
	add_trace(type='scatter', y=~tot_mass_kg, x=~YrMo, name='JVL', mode='lines+markers') %>%
	add_trace(type='scatter', y=~DN_total_kg, x=~YrMo, name='Naftz', mode='lines+markers') %>%
...
tn_mass_plot=plot_ly(data=gb_pool_data[gb_pool_data$param=='Total nitrogen',]) %>%
	add_trace(type='scatter', y=~tot_mass_kg, x=~YrMo, mode = 'lines+markers', name='Total') %>%
	add_trace(type='scatter', y=~DBL_mass_kg, x=~YrMo, mode = 'lines+markers', name='Deep brine layer') %>%
	add_trace(type='scatter', y=~UBL_mass_kg, x=~YrMo, mode = 'lines+markers', name='Upper brine layer') %>%
